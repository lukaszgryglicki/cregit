...
TOKENREPO=token
ORIGINALREPO=../original.repo/git

export BFG_MEMO_DIR=`pwd`/memo
BLAME_DIR=`pwd`/blame
EXTENSION='\.[ch]$'
HTML_DIR=`pwd`/html
COMMIT_URL=https://github.com/git/git/commit/
...
if [ "$CREATE_TOKEN_REPO" = "yes" ]; then 
    ...
    java -jar ${CREGIT}/jars/bfg.jar '--blob-exec:/home/dmg/git.dmg/cregit-scala/tokenizeByBlobId/tokenBySha.pl=\.[ch]$' --no-blob-protection
